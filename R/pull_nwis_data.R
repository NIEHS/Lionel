...
retry <- function(expr, max_tries, sleep_on_error = 0, verbose = FALSE, ...) {
  ...
  while (this_try <= max_tries) {
    ...
    result <- tryCatch(expr = do.call(expr, ...), error = function(ex) {
      message(paste0(ex, "\n"))
      return(data.frame())
    })
    ...
    if (is.data.frame(result) && nrow(result) == 0) {
      ...
      if (verbose && this_try < max_tries) {
        message(sprintf(
          "Attempt %s of %s unsuccessful, retrying...",
          this_try,
          max_tries
        ))
      }
      if (verbose && this_try == max_tries) {
        message(sprintf(
          "Attempt %s of %s unsuccessful, returning an empty data frame",
          this_try,
          max_tries
        ))
      }
      ...
      if (verbose)
        message(sprintf("Success! after attempt %s of %s", this_try, max_tries))
      break
    }
  }
  return(result)
